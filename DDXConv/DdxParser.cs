...
public class DdxParser(bool verbose = false)
{
    ...
    private void ConvertDdxToDds(BinaryReader reader, string outputPath, ConversionOptions options, uint magic)
    {
        ...
        D3DTextureInfo texture = ParseD3DTextureHeaderWithDimensions(textureHeader, out ushort width, out ushort height);
        ...
        if (isTwoChunkFormat)
        {
            ...
            int blockSize = texture.ActualFormat switch
            {
                // DXT1
            ...
            byte[] mips = UnpackMipAtlas(untiledAtlas, atlasWidth, atlasHeight, texture.ActualFormat, (int)texture.Width,
        ...
        else
        {
            ...
            if (mainData.Length > mainSurfaceSize)
            {
                ...
                if (width >= 512 && height >= 512)
                {
                    ...
                    while (mipWidth >= 4 && mipHeight >= 4 && processedMipData < remainingSize)
                    {
                        ...
                        if (processedMipData + mipSize > remainingSize)
                        {
                            break;
                        }
            ...
            else if (mainData.Length < mainSurfaceSize)
            {
                ...
                int estimatedAtlasSize = CalculateMipSize(width, height, texture.ActualFormat);  // Size of one surface at this resolution
                bool isLikelyAtlas = mainData.Length >= estimatedAtlasSize / 4 && mainData.Length <= estimatedAtlasSize;
                ...
                if (isLikelyAtlas && width >= 128 && height >= 128)
                {
                    ...
                    if (_verboseLogging)
                    {
                        Console.WriteLine($"Detected atlas-only data ({mainData.Length} bytes) - attempting mip extraction");
                    }
                    ...
                    if (_verboseLogging)
                    {
                        Console.WriteLine($"Using atlas dimensions: {atlasWidth}x{atlasHeight}");
                    }

                    // Untile the atlas
                    byte[] untiledAtlas = UnswizzleDXTTexture(mainData, atlasWidth, atlasHeight, texture.ActualFormat);
                    if (_verboseLogging)
                    {
                        Console.WriteLine($"Untiled atlas to {untiledAtlas.Length} bytes");
                    }
                    ...
                    if (_verboseLogging)
                    {
                        Console.WriteLine($"Reconstructing mips from atlas, largest: {largestMipWidth}x{largestMipHeight}");
                    }
                    ...
                    byte[]? topHalf = ExtractAtlasRegion(untiledAtlas, atlasWidth, atlasHeight,
                        0, 0, largestMipWidth, halfHeight, texture.ActualFormat);
                    byte[]? bottomHalf = ExtractAtlasRegion(untiledAtlas, atlasWidth, atlasHeight,
                    ...
                    {
                        byte[] mip0 = new byte[topHalf.Length + bottomHalf.Length];
                        ...
                        if (_verboseLogging)
                        {
                            Console.WriteLine($"  Mip 0: {largestMipWidth}x{largestMipHeight} ({mip0.Length} bytes) - reconstructed from split halves");
                        }
                        ...
                        {
                            byte[]? mipData = ExtractAtlasRegion(untiledAtlas, atlasWidth, atlasHeight,
                            ...
                            if (mipData != null)
                            {
                                ...
                                if (mipData.Length == expectedSize)
                                {
                                    ...
                                    if (_verboseLogging)
                                    {
                                        Console.WriteLine($"  Mip {mipCount - 1}: {mipW}x{mipH} at ({mipX},{mipY}) ({mipData.Length} bytes)");
                                    }
                                }
                            }
                        ...
                        if (_verboseLogging)
                        {
                            Console.WriteLine($"Reconstructed {mipCount} mip levels, total {totalMipSize} bytes (partial recovery from atlas)");
                        }
                    }
                    else
                    {
                        ...
                        if (_verboseLogging)
                        {
                            Console.WriteLine($"Could not reconstruct mips (topHalf={topHalf?.Length}, bottomHalf={bottomHalf?.Length}, expected={expectedHalfSize}), output full atlas");
                        }
                    ...
                    if (options?.SaveMips == true && outputPath != null)
                    {
                        ...
                        if (_verboseLogging)
                        {
                            Console.WriteLine($"Saved full atlas to {atlasPath}");
                        }
                    }

                    if (_verboseLogging)
                    {
                        Console.WriteLine($"Output {texture.Width}x{texture.Height} with {texture.MipLevels} mips (partial recovery from atlas-only data)");
                    }
                }
                else
                {
            ...
            else if (mainData.Length == mainSurfaceSize * 2)
            {
                ...
                uint halfMipChainSize = CalculateMipChainSize(halfWidth, halfHeight, texture.ActualFormat);
                ...
                byte[] fullUntiled = UnswizzleDXTTexture(mainData, width, height, texture.ActualFormat);
                ...
                if (couldBeMipAtlasForSmallerBase)
                {
                    ...
                    PackedMipAtlasResult? mipAtlasResult = TryExtractPackedMipAtlas(fullUntiled, width, height, halfWidth, halfHeight, texture.ActualFormat, _verboseLogging);

                    if (mipAtlasResult != null)
                    {
                        if (_verboseLogging)
                        {
                            Console.WriteLine($"Successfully extracted packed mip atlas: {halfWidth}x{halfHeight} base with {mipAtlasResult.MipCount} mip levels");
                        }
                ...
                if (!treatedAsMipAtlas)
                {
                    ...
                    byte[] mipsAlt = UnpackMipAtlas(chunk1UntiledAlt, width, height, texture.ActualFormat, width, height,
            ...
            else if (couldBeTwoSquares)
            {
                ...
                byte[] mips = UnpackMipAtlas(chunk1Untiled, squareSize, squareSize, texture.ActualFormat, squareSize,
            ...
            else
            {
                ...
                int blockSize = texture.ActualFormat is 0x82 or 0x52 or
                                0x7B
                    ? 8
                    : 16;
                ...
                if (width == 128 && height == 128 && mainData.Length == atlasSize128 + mainSize128)
                {
                    ...
                    // Extract mips from atlas
                    byte[] mips = UnpackMipAtlas(untiledAtlas, 256, 192, texture.ActualFormat, 128, 128, outputPath,
                ...
                else if (width >= 256 && height >= 256 && width == height)
                {
                    ...
                    PackedMipAtlasResult? mipAtlasResult = TryExtractPackedMipAtlas(fullUntiled, width, height, halfW, halfH, texture.ActualFormat, _verboseLogging);

                    if (mipAtlasResult != null && mipAtlasResult.MipCount >= 2)
                    {
                        if (_verboseLogging)
                        {
                            Console.WriteLine($"Detected packed mip atlas: {halfW}x{halfH} base with {mipAtlasResult.MipCount} mip levels in {width}x{height} tile space");
                        }
                    ...
                    else
                    {
                        ...
                        if (_verboseLogging)
                        {
                            Console.WriteLine($"Not a packed mip atlas, untiling as {width}x{height}");
                        }
    ...
    private static int CalculateMipSize(int width, int height, uint format) => (int)CalculateMipSize((uint)width, (uint)height, format);
    ...
    /// </summary>
    private static byte[]? ExtractAtlasRegion(byte[] atlasData, int atlasWidth, int atlasHeight,
    ...
    /// </summary>
    private static uint CalculateMipChainSize(int width, int height, uint format)
    {
        uint totalSize = 0;
        int w = width;
        int h = height;
        ...
        int baseWidth, int baseHeight, uint format, bool verbose)
    {
        ...
        _ = GetBlockSize(format);

        var mipDataList = new List<byte[]>();
        int totalMipSize = 0;

        // Extract base mip from top-left quadrant
        byte[]? baseMip = ExtractAtlasRegion(untiledData, atlasWidth, atlasHeight, 0, 0, baseWidth, baseHeight, format);
        if (baseMip == null)
        {
            return null;
        }
        ...
        if (verbose)
        {
            Console.WriteLine($"  Mip 0 (base): {baseWidth}x{baseHeight} at (0,0), {baseMip.Length} bytes");
        }
        ...
        {
            byte[]? mip1 = ExtractAtlasRegion(untiledData, atlasWidth, atlasHeight, mip1X, mip1Y, mip1W, mip1H, format);
            if (mip1 != null && mip1.Length == CalculateMipSize(mip1W, mip1H, format))
            {
                ...
                if (verbose)
                {
                    Console.WriteLine($"  Mip 1: {mip1W}x{mip1H} at ({mip1X},{mip1Y}), {mip1.Length} bytes");
                }
            }
        }
        ...
        {
            byte[]? mip2 = ExtractAtlasRegion(untiledData, atlasWidth, atlasHeight, mip2X, mip2Y, mip2W, mip2H, format);
            if (mip2 != null && mip2.Length == CalculateMipSize(mip2W, mip2H, format))
            {
                ...
                if (verbose)
                {
                    Console.WriteLine($"  Mip 2: {mip2W}x{mip2H} at ({mip2X},{mip2Y}), {mip2.Length} bytes");
                }
            }
        }
        ...
        {
            byte[]? mip3 = ExtractAtlasRegion(untiledData, atlasWidth, atlasHeight, mip3X, mip3Y, mip3W, mip3H, format);
            if (mip3 != null && mip3.Length == CalculateMipSize(mip3W, mip3H, format))
            {
                ...
                if (verbose)
                {
                    Console.WriteLine($"  Mip 3: {mip3W}x{mip3H} at ({mip3X},{mip3Y}), {mip3.Length} bytes");
                }
            }
        }
        ...
        {
            byte[]? mip4 = ExtractAtlasRegion(untiledData, atlasWidth, atlasHeight, mip4X, mip4Y, mip4W, mip4H, format);
            if (mip4 != null && mip4.Length == CalculateMipSize(mip4W, mip4H, format))
            {
                ...
                if (verbose)
                {
                    Console.WriteLine($"  Mip 4: {mip4W}x{mip4H} at ({mip4X},{mip4Y}), {mip4.Length} bytes");
                }
            }
        }
        ...
        {
            byte[]? mip5 = ExtractAtlasRegion(untiledData, atlasWidth, atlasHeight, mip5X, mip5Y, mip5W, mip5H, format);
            if (mip5 != null && mip5.Length == CalculateMipSize(mip5W, mip5H, format))
            {
                ...
                if (verbose)
                {
                    Console.WriteLine($"  Mip 5: {mip5W}x{mip5H} at ({mip5X},{mip5Y}), {mip5.Length} bytes");
                }
            }
        }
        ...
        if (mipDataList.Count < 2)
        {
            if (verbose)
            {
                Console.WriteLine("Could not extract enough mip levels from packed atlas");
            }
        ...
        // Combine all mips
        byte[] result = new byte[totalMipSize];
        int offset = 0;
        foreach (byte[] mip in mipDataList)
        {
        ...
        if (verbose)
        {
            Console.WriteLine($"  Total: {mipDataList.Count} mip levels, {totalMipSize} bytes");
        }
        ...
        int height, uint format)
    {
        ...
        int blockSize = format is 0x82 or 0x52 or 0x7B ? 8 : 16;
    ...
    private static bool GetPackedMipOffset(int texWidth, int texHeight, int mipLevel, out int xBlocks, out int yBlocks)
    {
        ...
        string? outputPath = null, bool saveMips = false)
    {
        ...
        if (width == 1024 && height == 1024)
        {
            ...
            if (saveMips && outputPath != null)
            {
                try
                {
                    ...
                    byte[] mipData0 = new byte[mipByteCount0];
                    ...
                    string mipPath0 = outputPath.Replace(".dds", "_mip0.dds");
                ...
                catch (Exception ex)
                {
                    if (_verboseLogging)
                    {
                        Console.WriteLine($"Failed to save mip 0: {ex.Message}");
                    }
                }
            }
            ...
            if (saveMips && outputPath != null)
            {
                try
                {
                    ...
                    byte[] mipData1 = new byte[mipByteCount1];
                    ...
                    string mipPath1 = outputPath.Replace(".dds", "_mip1.dds");
                ...
                catch (Exception ex)
                {
                    if (_verboseLogging)
                    {
                        Console.WriteLine($"Failed to save mip 1: {ex.Message}");
                    }
                }
            }
            ...
            {
                (int mipX, int mipY, int mipW, int mipH) = remainingMips[i];
                ...
                if (saveMips && outputPath != null)
                {
                    try
                    {
                        ...
                        byte[] mdata = new byte[thisMipSize];
                        ...
                        string mpath = outputPath.Replace(".dds", $"_mip{i + 2}.dds");
                    ...
                    catch (Exception ex)
                    {
                        if (_verboseLogging)
                        {
                            Console.WriteLine($"Failed to save small mip {i + 2}: {ex.Message}");
                        }
                    }
                }
            }
        ...
        (int x, int y, int w, int h)[] mipPositions = width == 256 && height == 192 ?
            [
        ...
        foreach ((int x, int y, int w, int h) in mipPositions)
        {
        ...
        if (!mappingFits)
        {
            ...
            for (int i = 0; i < mips; i++)
            {
                ...
                if (mbW > atlasWBlocks || mbH > atlasHBlocks)
                {
                    break; // can't place this mip
                }
        ...
        {
            (int mipXInBlocks, int mipYInBlocks, int mipWidthInBlocks, int mipHeightInBlocks) = mipPositions[mipLevel];
            ...
            if (saveMips && mipByteCount > 0 && outputPath != null)
            {
                try
                {
                    byte[] mipData = new byte[mipByteCount];
                    ...
                    string mipPath = outputPath.Replace(".dds", $"_mip{mipLevel}.dds");
        ...
        // Trim to actual extracted size
        byte[] finalTrimmed = new byte[outputOffset];
    ...
    {
        int blockSize = format is 0x52 or 0x82 or 0x12 ? 8 : 16;
        ...
        for (int col = 0; col < 3; col++)
        {
            ...
            for (int r = 0; r < rectCount; r++)
            {
                ...
                if (rectIdx < 0 || rectIdx >= _pTextureTileRectsA.Length)
                {
                    continue;
                }

                TextureTileRectDef rect = _pTextureTileRectsA[rectIdx];
